...
export function SidebarComponent({ className, isCollapsed, links }: SidebarProps) {
  ...
  return (
    <div
      ...
      className={cn("group flex flex-col h-full gap-4 py-2 data-[collapsed=true]:py-2", className)}
    >
      <nav className="grid gap-1 px-2 group-[[data-collapsed=true]]:justify-center group-[[data-collapsed=true]]:px-2">
        <ScrollArea className="h-[calc(100vh-80px)]">
          {links.map((link, index) => {
            const Icon = iconComponents[link.icon.displayName || link.icon.name] || HomeIcon // Fallback to HomeIcon
            return isCollapsed ? (
