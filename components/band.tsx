...
export default function Band({ maxSpeed = 50, minSpeed = 10 }) {
  const band = useRef<THREE.Mesh>(null) // Changed type to THREE.Mesh
  ...
  const texture = useTexture("/assets/images/lanyard_texture.png") // Using new file name to bypass cache

  // Force texture to load from local path
  useEffect(() => {
    if (texture) {
      console.log("Texture loaded from:", texture.image?.src || "unknown source")
      // Force local texture loading
  ...
  // Explicitly create MeshLineGeometry
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      // Use the memoized geometry and its setPoints method
      meshLineGeom.setPoints(curve.getPoints(32))
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        <primitive object={meshLineGeom} attach="geometry" /> {/* Attach memoized geometry */}
        <meshLineMaterial
          color="white"
          depthTest={false}
